...
- Ekran gry na desktopie mieści się w jednym oknie (`.game { height: 100dvh }`). Elastyczny jest wiersz dziennika: to on bierze resztę miejsca, a przewijają się tylko dwie rzeczy, `.panel-body` i `.journal-list`. Zdjęcie miejsca, kwestia postaci i przycisk końca tygodnia zostają na wierzchu. Element rozpięty na kilka wierszy (panel) musi mieć `align-self: stretch` i `min-height: 0`, inaczej rośnie z treścią i wypycha stronę.
