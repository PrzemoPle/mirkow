...
describe("parseSave", () => {
  ...
  it("rejects broken JSON and unknown versions", () => {
    ...
    expect(parseSave(JSON.stringify({ version: 4, state: createMatch() }))).toEqual({
      status: "corrupt",
    });
  ...
  it("rejects a playing match with no players", () => {
    ...
    expect(
      parseSave(JSON.stringify({ version: 3, state: broken })),
...
describe("writeSave", () => {
  it("clears the slot on request", () => {
    ...
    const store = memoryStore({
      [SAVE_KEY]: JSON.stringify({ version: 3, state: match }),
    });
